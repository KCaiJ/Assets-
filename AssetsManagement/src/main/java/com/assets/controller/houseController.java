...
public class houseController {  
    ...
    public String getAll(HttpServletRequest request,Model model, HttpSession session){  
    	try {
            List<house> user = Service.findAll((int) session.getAttribute("useoId"));  
   ...
   public String get(int id,HttpServletRequest request,Model model){  
    ...
    public void update(HttpServletResponse resp,house user,HttpServletRequest request,Model model){  
    	try {
    		System.err.println(user.toString());
      	  String jsonStr="";
      	  house olduser = Service.findById(user.getDid());
      	System.err.println(olduser.toString());
      	  olduser.sethouselaction(user.gethouselaction());
      	  olduser.sethousearea(user.gethousearea());
    ...
    public void insert(house user,Model model,HttpServletResponse resp, HttpSession session){  
    	...
    	try {
    		 ...
    		 user.setStatus(1);
    		 user.sethousecode(UUID.randomUUID().toString());
    ...
    public String name(Model model,int id, HttpSession session) {
    	...
    	return "/house/house/out";  
	}
    
    @RequestMapping("/acceptout.do")  
    public void accep(Model model,houseout booksout,HttpServletResponse resp, HttpSession session){
    	String jsonStr="{\"code\":\"1001\",\"msg\":\"操作失败\"}"; 
    	try {
    		 User u=(User) session.getAttribute("user"); 
